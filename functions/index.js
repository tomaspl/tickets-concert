...
exports.testScheduler = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    if (stageIndex === -1 && items.length > 0) {
      ...
      console.log(
        `ℹ️ No había nadie en stage. Asignando onStageAt a ${items[0].familyId || items[0].key}`,
      )
    ...
    if (stageIndex !== -1) {
      ...
      if (current.onStageAt && now - current.onStageAt > twoMinutes) {
        ...
        await queueRef.update(updates)
        console.log(
          `🚪 Expulsada familia ${expelledFamilies.join(',')}. onStageAt asignado al siguiente en cola (si existía).`,
        )
        return null
      } else {
